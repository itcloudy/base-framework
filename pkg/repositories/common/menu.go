...
// 查询菜单
func (repo *MenuRepository) FindAllMenu(DB *gorm.DB, page, size int, order string, query string, queryArgs ...interface{}) (results []*models.MenuList, total int, err error) {
	if len(order) == 0 {
		order = "id desc"
	}
	db := DB.Model(&models.MenuList{}).Order(order)
	if len(query) > 0 {
